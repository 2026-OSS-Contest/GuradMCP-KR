...
- **Gateway**(`packages/gateway/src`): `server.ts`의 `tools/call` 처리에서, 도구가 `send_email`이고 `body`가 문자열이면 탐지·마스킹 대상 텍스트를 `subject`+`body`로 한정한다(탐지·정책 판단은 구분자로 이어붙인 하나의 텍스트에 대해 수행하되, 마스킹 후 되돌릴 때는 그 텍스트를 다시 파싱하지 않는다 — 탐지된 구간의 오프셋을 `subject`/`body` 각자의 길이 기준으로 나눠, 각 필드를 원문에 대해 독립적으로 마스킹한다. 마스킹된 결과 텍스트에서 구분자를 다시 찾는 방식은 두 가지로 깨진다: 마스킹된 구간이 구분자 자체를 집어삼킬 수 있고, `subject`에 실제 개행이 들어 있으면 그게 구분자로 오인된다 — 둘 다 재현해 확인한 뒤 오프셋 기반 방식으로 고쳤다. 탐지 구간이 경계를 걸치는 경우(PEM 키처럼 `subject` 끝에서 시작해 `body`로 이어지는 패턴)도 실제로 있어, 그런 탐지는 둘로 나눠 각 필드에 걸친 부분만 마스킹한다) — `to` 주소는 PII 탐지기가 `PII.EMAIL`로 잡아내는 값이라 여전히 검사 대상에서 제외한다: 인자 전체 JSON을 검사·마스킹 대상으로 삼으면 마스킹 후 승인 시 수신자 주소 자체가 `[EMAIL]`로 깨진다(이 문제는 실제로 재현해 확인했다). `subject`를 포함시키는 이유는 그 반대 실패 모드다 — `body`만 검사하면 `subject`에 실은 비밀/PII는 탐지 0건으로 조용히 통과해 마스킹도 승인도 거치지 않고 나간다(리뷰에서 지적, 재현 확인 후 수정). 마스킹된 결과는 원래의 `to`와 다시 합쳐서 upstream으로 보낸다. `CONTROL_PLANE_URL`이 설정되면 `server.ts`가 `controlPlane/approvalBackend.ts`를 승인 백엔드로 선택한다 — 실제 Control Plane에 승인을 등록하고(`POST /api/v1/approvals`) 폴링으로 결정을 기다리며(§10: "SSE 또는 polling" 중 polling 경로), Control Plane에 도달할 수 없거나 응답이 없으면 로컬 데드라인으로 fail-closed된다. `CONTROL_PLANE_URL`이 없으면 기존의 즉시-만료 백엔드(`approval/backend.ts`)로 fail-closed 동작을 유지한다. 승인 카드에 필요한 위험 태그·마스킹 미리보기는 `pipeline/approvalPreview.ts`가 대기 중인 요청 자체에서 계산해 `actionRouter.ts`의 `awaitApproval()`이 함께 전송한다(원문은 아직 아무 데도 전송되지 않은 상태이므로 이 시점엔 정당하게 보관 가능 — NFR-04, 마스킹 미리보기는 정책이 `allow_masked_approval`을 허용할 때만 보낸다). 승인 카드의 `arguments`는 `body`를 뺀 나머지 전부가 아니라 `to`/`subject`만 담는 화이트리스트다(리뷰에서 지적: 이전에는 `body`만 제외하는 필터라 `cc`·첨부 등 임의 인자가 그대로 Control Plane 레코드로 넘어갔고, 문자열이 아닌 값은 Control Plane의 `Map<String, String>` 바인딩에 실패해 승인 자체가 `APPROVAL_TIMEOUT_BLOCKED`로 깨졌다 — 재현 확인 후 수정). `body`를 카드에 다시 넣지 않는 이유는 그대로다 — 이미 `maskPreview`가 (수명이 있는 채로) 담고 있어서, 같은 원문을 `arguments`로도 보내면 아무도 지우지 않는 사본이 하나 더 생긴다. send_email이 아닌 다른 도구는 애초에 `arguments`를 보내지 않는다. GuardEvent를 Control Plane 감사 로그(`POST /api/v1/events`)로 전달하는 것은 이 티켓 이전부터 있던 `pipeline/auditPublisher.ts`(GMCP-24)가 이미 하고 있어 새로 만들지 않았다.
...
- **send_email의 `argsDigest`는 이제 `subject`+구분자+`body`(조인된 검사 텍스트)의 다이제스트다.** 다른 모든 도구는 여전히 인자 전체 JSON의 다이제스트를 쓰므로, send_email만 그 의미가 다르다(§8.4). `subject`가 없는 호출은 구분자+`body`만 다이제스트한다. 감사 추적에서 이 도구만 다른 기준으로 다이제스트가 계산된다는 점을 아직 별도로 표시하지 않는다 — 리뷰에서 지적된 비차단 항목으로, 이번 수정 범위에는 포함하지 않았다.
