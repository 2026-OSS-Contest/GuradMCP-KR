...
* `--dry-run-only` (SPEC-POL-04 §7.1, GMCP-77): instead of `runBenchmark()`'s fixed KPI evaluation, replays a labeled-normal (`label: false`) dataset with `evaluationMode: "shadow-all"` and reports each policy's FPR — every matched policy is treated as shadow regardless of its own `dry_run` value, so nothing is ever acted on. `--dataset <path>`: a JSON file shaped `{id, label, text}[]` (default: the `label: false` samples in `attack-lab/datasets/pii-benchmark.json` — the design doc's own `attack-lab/datasets/normal-kr-v1` isn't shipped in this repo yet). `--fail-on-fpr <0..1>` (§7.2's CI gate): exits non-zero if any policy's FPR exceeds the threshold, and prints the offending policies to stderr. When `CONTROL_PLANE_URL` is set, each policy's result is also posted to `POST /api/v1/policies/{id}/benchmark-results` (best-effort — a failed post only warns, since CI has no Control Plane to reach).
...
* Exit code: `bench run` is non-zero if the report's `passed` is false. `bench run --dry-run-only` is non-zero if any policy crosses `--fail-on-fpr`. `bench compare` is non-zero if either condition above is hit.
...
* **No `--dataset`/`--policy-pack` on `bench run`'s default mode**: `runBenchmark()` always evaluates the entire `policy-packs/` tree against a fixed dataset bundle and has no subsetting hook. Rather than accept flags that would be silently ignored, they are not exposed. `--dry-run-only` (GMCP-77) is a separate evaluation path, so it accepts `--dataset` independent of this constraint.
