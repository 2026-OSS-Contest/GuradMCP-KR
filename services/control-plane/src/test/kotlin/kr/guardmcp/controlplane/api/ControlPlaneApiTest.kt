...
/** `security.sync-token` must be configured for `POST /policies/sync` to succeed — see
 *  [PolicyController.hasValidSyncToken]'s doc comment: unconfigured (blank) always denies. */
...
class ControlPlaneApiTest : ApiTestSupport() {
    ...
    @BeforeEach
    fun syncPolicies() {
        val response = syncPolicyRegistry()
        assertEquals(200, response.statusCode())
    }

    private fun syncPolicyRegistry(): HttpResponse<String> =
        send("POST", "/api/v1/policies/sync", PolicyFixtures.syncRequestBody(), headers = mapOf(SYNC_TOKEN_HEADER to "test-sync-token"))
