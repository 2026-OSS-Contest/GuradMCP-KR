...
export const handlers = [
  ...
  http.get("*/api/v1/servers", async () =>
    // `currentServers()` rather than the `SERVERS` fixture: SCR-501 can change a server's trust,
    // so the inventory has to come from the mutable copy the settings mock keeps.
