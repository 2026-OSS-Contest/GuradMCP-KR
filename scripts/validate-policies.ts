...
const approvalFields = new Set(["timeout_seconds", "on_timeout", "allow_masked_approval"]);
...
for (const packName of await directoryNames(root)) {
  ...
  if (manifest) {
    ...
    if (typeof manifest.version !== "string" || !/^\d+\.\d+\.\d+$/.test(manifest.version)) failures.push(`${manifestPath}: version must be semantic x.y.z`);
    ...
    for (const relative of Array.isArray(manifest.policies) ? manifest.policies : []) {
      if (typeof relative !== "string" || !relative.startsWith("policies/") || normalize(relative).startsWith("..")) {
        failures.push(`${manifestPath}: invalid policy path ${String(relative)}`);
      } else {
        if (listedPolicyPaths.has(relative)) failures.push(`${manifestPath}: duplicate policy path ${relative}`);
        listedPolicyPaths.add(relative);
        if (!await isFile(join(packPath, relative))) failures.push(`${manifestPath}: missing policy file ${relative}`);
      }
    }
    if (manifest.extends !== undefined && (!Array.isArray(manifest.extends) || !manifest.extends.every((entry) => typeof entry === "string"))) failures.push(`${manifestPath}: extends must be a string list`);
  }
  ...
  for (const path of await policyFiles(packPath)) {
    ...
    if (!listedPolicyPaths.has(manifestRelative)) failures.push(`${path}: policy file must be listed in pack.yaml`);
...
for (const [packName, manifest] of packs) {
  for (const reference of Array.isArray(manifest.extends) ? manifest.extends : []) {
    ...
    if (!parsed) {
      failures.push(`${join(root, packName, "pack.yaml")}: invalid extended pack constraint ${String(reference)}`);
      continue;
    }
    ...
    if (typeof parentVersion === "string" && Number(parentVersion.split(".")[0]) !== minimum[0]) failures.push(`${join(root, packName, "pack.yaml")}: incompatible extended pack ${String(reference)} (found ${parentVersion})`);
  }
}

function parseExtend(reference: string): { parent: string; minimum: [number, number, number] } | undefined {
  const match = /^([a-z0-9][a-z0-9-]*)@\^(\d+)\.(\d+)\.(\d+)$/.exec(reference);
  return match ? { parent: match[1] as string, minimum: [Number(match[2]), Number(match[3]), Number(match[4])] } : undefined;
}
...
function validateMatch(path: string, value: unknown): void {
  ...
  if (value.detections !== undefined && !isRecord(value.detections)) failures.push(`${path}: match.detections must be an object`);
    ...
    rejectUnknownFields(`${path}: match.detections`, value.detections, detectionFields);
    for (const [key, tags] of Object.entries(value.detections)) {
      if (!Array.isArray(tags) || tags.length === 0 || !tags.every((tag) => typeof tag === "string" && tag.length > 0)) failures.push(`${path}: match.detections.${key} must be a non-empty string list`);
    }
  }
  if (isRecord(value.risk_score)) {
    rejectUnknownFields(`${path}: match.risk_score`, value.risk_score, riskFields);
...
function validateArgs(path: string, args: Record<string, unknown>): void {
  for (const [condition, value] of Object.entries(args)) {
    if (condition.endsWith("_exists")) {
      if (typeof value !== "boolean") failures.push(`${path}: match.args.${condition} must be boolean`);
    } else if (condition.endsWith("_regex")) {
      if (typeof value !== "string" || value.length === 0 || value.length > 512) {
        failures.push(`${path}: match.args.${condition} must be a 1..512 character regex string`);
      } else if (!isSafePolicyRegex(value)) failures.push(`${path}: match.args.${condition} is not in the safe JavaScript regex subset`);
    } else if (condition.endsWith("_glob")) {
      if (typeof value !== "string" || value.length === 0) failures.push(`${path}: match.args.${condition} must be a non-empty glob string`);
    } else if (condition.endsWith("_domain") || condition.endsWith("_not_domain")) {
      const domains = Array.isArray(value) ? value : [value];
      if (domains.length === 0 || !domains.every((domain) => typeof domain === "string" && /^[a-z0-9.-]+$/i.test(domain))) failures.push(`${path}: match.args.${condition} must contain valid domains`);
    } else if (condition.endsWith("_in") || condition.endsWith("_not_in")) {
      if (!Array.isArray(value) || value.length === 0) failures.push(`${path}: match.args.${condition} must be a non-empty list`);
    } else if (typeof value === "object" && value !== null) {
      failures.push(`${path}: match.args.${condition} exact values must be scalar`);
    }
  }
}
...
function validateExtendsCycles(): void {
  ...
  const visit = (packName: string, trail: string[]): void => {
    if (visiting.has(packName)) {
      failures.push(`${join(root, packName, "pack.yaml")}: extends cycle ${[...trail, packName].join(" -> ")}`);
      return;
    }
...
function rejectUnknownFields(path: string, value: Record<string, unknown>, allowed: Set<string>): void {
  for (const field of Object.keys(value)) if (!allowed.has(field)) failures.push(`${path}: unknown field ${field}`);
}
